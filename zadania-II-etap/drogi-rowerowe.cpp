...
int countReachable(int sccId, vector <vector<int>> &adj, vector<bool> &visited, vector<int> &sccReachCount) {
    if (visited[sccId]) return sccReachCount[sccId]; //nie musimy przeprowadzac kolejnego dfsa na tym samym scc, bo kazy node w scc ma tyle samo reachable nodes
    visited[sccId] = true;

    int reachableNodes = sccReachCount[sccId]; // Start by counting the nodes in this SCC itself
    for (int nextSccId: adj[sccId]) {
        reachableNodes += countReachable(nextSccId, adj, visited, sccReachCount);
    }

    return sccReachCount[sccId] = reachableNodes;
}
...
int main() {
    ...
    vector<int> sccReachCount(SCCs.size());  // dp table for reach count
