...
ll calculateManhattanSum(vector<ll> &positions) {
    ...
    vector<ll> dp(n, 0);
    dp[0] = 0;
    ll sum_before = 0, count_before = 0;
    ...
    for (ll i = 0; i < n; i++) {
        ...
        //zaluzmy ze mamy |x1-x2| + |x1-x3| + |x1-x4|, skoro x1 jest wieksze od x2,x3,x4 to x1-x2 + x1-x3 + x1-x4 = 3x1 - x2 - x3 - x4   3x1 to count before*positions[i], a druga czesc rownania to sum_before
        //(count_before * positions[i] - sum_before) - to jest aktualna suma miedzy wszystkimi parami punnktow z xi w gore tablicy
        if (i != 0) {
            dp[i] = dp[i - 1] + (count_before * positions[i] - sum_before);
        }
        //prefix sum
